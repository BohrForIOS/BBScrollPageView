...
@interface BBScrollPageView ()<BBScrollContentViewDelegate>
/**
 *  标题视图的风格
 */
@property (nonatomic, strong) BBScrollTitleViewStyle *titleViewStyle;
/**
 *  scrollTitleView(外面只读，可供使用)
 */
@property (nonatomic, strong) BBScrollTitleView *scrollTitleView;
/**
 *  scrollContentView(外面只读，可供使用)
 */
@property (nonatomic, strong) BBScrollContentView *scrollContentView;
...
@implementation BBScrollPageView
...
- (void)p_initScrollTitleView {
    _scrollTitleView = [[BBScrollTitleView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44) style:self.titleViewStyle titles:self.titlesArray titleDidClick:^(UILabel *label, NSInteger index) {
...
- (void)p_initScrollContentView {
    _scrollContentView = [[BBScrollContentView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_scrollTitleView.frame), self.frame.size.width, self.frame.size.height - CGRectGetHeight(self.scrollTitleView.frame)) parentViewController:self.parentViewController childVcs:self.childVcs delegate:self];
...
- (void)scrollContentView:(BBScrollContentView *)scrollContentView didScrollToIndex:(NSInteger)index {
    ...
    // 让对应的顶部标题居中显示
    BBScrollTitleMenuLabel*label = self.scrollTitleView.menuLabels[index];
    ...
    // 让其他label回到最初的状态
    for (BBScrollTitleMenuLabel *otherLabel in self.scrollTitleView.menuLabels) {
        if (label != otherLabel)  otherLabel.scale = 0;
    }
}
...
- (void)scrollContentView:(BBScrollContentView *)scrollContentView contentOffsetScaleWhileScroll:(CGFloat)scale {
    ...
    BBScrollTitleMenuLabel *leftLabel = self.scrollTitleView.menuLabels[leftIndex];
    
    // 获得需要操作的右边label
    NSInteger rightIndex = leftIndex + 1;
    BBScrollTitleMenuLabel *rightLabel = (rightIndex == self.scrollTitleView.menuLabels.count) ? nil : self.scrollTitleView.menuLabels[rightIndex];
